...
- `SupportBoard-Testserver.html`: dritte Ausgabe für den Serverbetrieb. Technisch identisch mit der Testversion SQL (KANAL bleibt "sqltest": gleicher Browser-Speicher, nimmt die kopierte `SupportBoard-Team-SQLTest.json` an), nur Beschriftung: Titel/Name „Testserver-Version“, Badge „TEST · TESTSERVER“. Wird aus `SupportBoard-SQLTest.html` per Textersetzung gebaut (Titel, KANAL_INFO.sqltest name/badge, kanalName, Parallelbetrieb-Text). VKU legt sie im Zielordner als `SupportBoard.html` ab.
